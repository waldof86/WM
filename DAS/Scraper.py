...
from utils import Profile, ScrapingException, is_url_valid, HumanCheckException, wait_for_loading, wait_for_scrolling, \
    Job, AuthenticationException, Location, Company, ScrapingResult, Degree
...
class Scraper(Thread):
    ...
    def scrape_profile_name(self):
        return self.browser.execute_script(
            "return document.getElementsByClassName('pv-text-details__left-panel')[0].children[0].innerText")
    ...
    def load_full_page(self):
        ...
        while self.browser.execute_script(
                "return document.getElementsByClassName('pv-profile-section__see-less-inline').length") < 1 and self.browser.execute_script(
                "return document.getElementsByClassName('pv-profile-section__section-info--has-no-more').length") < 1:
            try:
                self.browser.execute_script(
                    "document.getElementsByClassName('pv-profile-section__see-more-inline')[0].click()")
               
            except WebDriverException as err:
                print("Page full load error: " + err)
                pass
            wait_for_loading()
...
#My code extension
    def scrape_education(self):
        ...
        try:
            degrees = []
            degrees = self.browser.execute_script(
                """
             return (function() {
                var degrees = [];
                var els = document.getElementById('education-section').getElementsByTagName('ul')[0].getElementsByTagName('li');
                for (var i = 0; i < els.length; i++) {
                    try {
                        school_name= els[i].getElementsByClassName('pv-entity__summary-info')[0].getElementsByTagName('h3')[0].innerText;
                    } catch (err) {
                        school_name = '';
                    }
                    try {
                        degree = els[i].getElementsByClassName('pv-entity__summary-info')[0].getElementsByClassName('pv-entity__secondary-title')[0].getElementsByTagName('span')[1].innerText;
                    } catch (err) {
                        degree = '';
                    }
                    try {
                        date_ranges = els[i].getElementsByClassName('pv-entity__summary-info')[0].getElementsByClassName('pv-entity__date-range')[0].getElementsByTagName('span')[1].innerText;
                    } catch (err) {
                        date_ranges = '';
                    }
                    try {
                        company_url = els[i].getElementsByTagName('a')[0].href;
                    } catch (err) {
                        company_url = '';
                    }
                    try {
                        fields = els[i].getElementsByClassName('pv-entity__summary-info')[0].getElementsByClassName('pv-entity__fos')[0].getElementsByTagName('span')[1].innerText;
                    } catch (err) {
                        fields = '';
                    }    
                    degrees.push([degree, school_name, company_url, date_ranges, fields]);
                }
                return degrees;
            })();
                """
