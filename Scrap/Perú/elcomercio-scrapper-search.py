...
headless = False
url = 'https://elcomercio.pe/buscar/pandemia/todas/descendiente/'

headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'}
pagen = 1
page= requests.get(url+ str(pagen)+'/',headers=headers)               
...
while (p_actual <= p_final):
    ...
    for n in np.arange(0, number_of_articles):
            ...
            list_links.append('https://elcomercio.pe/'+link) 
            
    
    
            # Ir al artículo (it is divided in paragraphs)
            print('https://elcomercio.pe/'+link)
        
            try:
                #article
                headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36'}
                article = requests.get('https://elcomercio.pe/'+link,headers=headers)
                #time.sleep(2)
                article_content = article.content
                soup_article = BeautifulSoup(article_content, 'html5lib')
                
                try:
                    seccion = soup_article.find('a', class_='st-social__link').get_text() 
                ...
                try:
                    autor = soup_article.find('a', class_='story-contents__author-link').get_text()
                    list_autores.append(autor)
                except:
                    list_autores.append('')
                
                try:
                    fecha = soup_article.find('time', class_='story-contents__time')['datetime']    
                except:
                    fecha=''
...
workbook = xlsxwriter.Workbook('elcomercio-results.xlsx')
